...
- Закрыт frontend scope USR-FLT-03: добавлены quick chips `Только активные`, `Только логисты`, `Только менеджеры`, `Без компании`; chips комбинируются с основной формой, сбрасывают `page=1`, раскрытие фильтра синхронизируется через `filters_open`.
- Полное серверное применение `is_active=true` и `has_company=false` остаётся backend-зависимостью `CRM-68`; фронт не имитирует фильтрацию локально по текущей странице.
