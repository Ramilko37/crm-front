...
vi.mock("@/server/bff/orchestration", () => ({
  buildClientOrderMultipartPayload: vi.fn(),
}));

describe("POST /api/client/orders route", () => {
  beforeEach(() => {
  ...
  it("proxies canonical multipart body as-is", async () => {
    ...
    const request = new Request("http://localhost/api/client/orders", {
      method: "POST",
      headers: {
    ...
    expect(proxyToBackend).toHaveBeenCalledWith(request, "/client/orders");
